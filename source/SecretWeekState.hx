...
class ArcWeekData {
	public static var universeweeksLoaded:Map<String, ArcWeekData> = new Map<String, ArcWeekData>();
	public static var universeweeksList:Array<String> = [];
	...
	public static function reloadArcWeekFiles(isArcStoryMode:Null<Bool> = false)
	{
		...
		for (i in 0...sexList.length) {
			for (j in 0...directories.length) {
				...
				if(!universeweeksLoaded.exists(sexList[i])) {
					...
					if(Arcweek != null) {
						...
						if(ArcweekFile != null && (isArcStoryMode == null || (isArcStoryMode && !ArcweekFile.hideStoryMode) || (!isArcStoryMode && !ArcweekFile.hideFreeplay))) {
							universeweeksLoaded.set(sexList[i], ArcweekFile);
							universeweeksList.push(sexList[i]);
						}
					}
				}
			}
	...
	{
		if(!universeweeksLoaded.exists(ArcweekToCheck))
		{
			...
			if(week != null)
			{
				...
				{
					universeweeksLoaded.set(ArcweekToCheck, ArcweekFile);
					universeweeksList.push(ArcweekToCheck);
				}
			}
		}
	}
	...
	public static function getArcWeekFileName():String {
		return universeweeksList[PlayState.storyWeek];
	}

	//Used on LoadingState, nothing really too relevant
	public static function getCurrentArcWeek():ArcWeekData {
		return universeweeksLoaded.get(universeweeksList[PlayState.ArcstoryWeek]);
	}
